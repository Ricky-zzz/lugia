...
{
    ?>
    <form id="filterForm" method="get" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>"
        ...
        <div class="flex-grow-1">
            <?php foreach ($fields as $field):
                ...
                $value = $_GET[$name] ?? ($field['value'] ?? '');
                $enabled = !empty($value);
                $checkboxId = "enable" . ucfirst($name);
                $inputId = $name . "Filter";

                // 👇 handle hidden separately
                if ($type === 'hidden'): ?>
                    <input type="hidden" id="<?= $inputId ?>" name="<?= htmlspecialchars($name) ?>"
                ...
                <div class="mb-3">
                    ...
                    <?php if ($type === 'select'): ?>
                        <select class="form-select form-select-sm mt-2" id="<?= $inputId ?>" name="<?= htmlspecialchars($name) ?>"
                            <?= !$enabled ? 'disabled' : '' ?>>
                            <option value="">-- Select --</option>
                            <?php foreach ($field['options'] as $val => $txt): ?>
                                <option value="<?= htmlspecialchars($val) ?>" <?= $value == $val ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($txt) ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php else: ?>
                        <input type="<?= $type ?>" class="form-control form-control-sm mt-2" id="<?= $inputId ?>"
                            name="<?= htmlspecialchars($name) ?>" value="<?= htmlspecialchars($value) ?>"
                            placeholder="<?= htmlspecialchars($placeholder) ?>" <?= !$enabled ? 'disabled' : '' ?>>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
            ...
            <div class="d-grid gap-2 mt-3">
                ...
                <a href="<?= strtok($_SERVER['REQUEST_URI'], '?') ?>" class="btn btn-outline-secondary btn-sm"
                    id="clearFilters">
